...
export let Controls = function(object, domElement) {
    ...
    this.panTo = (vec3) => {
        const newTarget = new THREE.Vector3(vec3.x, scope.target.y, vec3.z);
        const delta = scope.target.clone().sub(newTarget);
        pan.sub(delta);
        scope.update();
    };
