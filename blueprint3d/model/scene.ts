...
export class Scene {
     ...
     */
    public addItem(itemType: number, fileName: string, metadata,
                   position: THREE.Vector3, rotation: number, scale: THREE.Vector3, fixed: boolean) {
        itemType = itemType || 1;
        const scope = this;
        const loaderCallback = (geometry: THREE.Geometry, materials: THREE.Material[]) => {
